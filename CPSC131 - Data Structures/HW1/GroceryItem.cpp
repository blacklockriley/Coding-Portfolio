...
{
  ///////////////////////// TO-DO (15) //////////////////////////////
  /// Copy assignment "works" but is not correct.  Be sure to move newUpcCode
  /// into _upcCode
...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (19) //////////////////////////////
  if (auto cmp = _upcCode.compare(rhs._upcCode); cmp != 0) return cmp <=> 0;

  if (auto cmp = _productName.compare(rhs._productName); cmp != 0)
    return cmp <=> 0;

  if (auto cmp = _brandName.compare(rhs._brandName); cmp != 0) return cmp <=> 0;
...
bool GroceryItem::operator==( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (20) //////////////////////////////
  return (floating_point_is_equal(_price, rhs._price) && _upcCode == rhs._upcCode && _brandName == rhs._brandName &&
          _productName == rhs._productName) ;
  /////////////////////// END-TO-DO (20) ////////////////////////////
}
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  /// effects if an error or exception occurs, so let's do our work in a local
  /// object and move it into place at the end if all goes well.
  ///
  /// This function should be symmetrical with operator<< below.  Read what your
  /// write, and write what you read
  ///
  ///
  /// Assume fields are separated by commas & optional spaces, and string
  /// attributes are enclosed with double quotes.  For example:
  ///    UPC Code         | Brand Name | Product Name | Price
  ///    -----------------+------------+---------------------------------------------------------------+-------
  ///    "00034000020706",  "York",      "York Peppermint Patties Dark Chocolate
  ///    Covered Snack Size"  ,  12.64
  ///
  ///
  ...
  if (stream >> std::ws >> std::quoted(tempItem._upcCode) &&
      stream >> std::ws >> delimiter && delimiter == ',' &&
      stream >> std::ws >> std::quoted(tempItem._brandName) &&
      stream >> std::ws >> delimiter && delimiter == ',' &&
      stream >> std::ws >> std::quoted(tempItem._productName) &&
      stream >> std::ws >> delimiter && delimiter == ',' &&
      stream >> std::ws >> tempItem._price) {
...
{
  ///////////////////////// TO-DO (22) //////////////////////////////
  /// This function should be symmetrical with operator>> above.  Read what your
  /// write, and write what you read
  ///
  /// Hint:  Brand and product names may have quotes, which need to escaped when
  /// printing.  Use std::quoted to read and write quoted strings.  See
  ///        1) https://en.cppreference.com/w/cpp/io/manip/quoted
  ///        2) https://www.youtube.com/watch?v=Mu-GUZuU31A
