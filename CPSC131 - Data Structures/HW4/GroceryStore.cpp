...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  std::string  upc; // UPC code
  unsigned int quantity; // Quantity on hand

  while( fin >> std::quoted( upc ) >> quantity ) _inventoryDB[upc] = std::move(quantity); // Read inventory records until end of file, adding each record to the memory resident inventory database
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomers( const ShoppingCarts & shoppingCarts, std::ostream & receipt )
{
  ...
  for( const auto & [name, shoppingCart] : shoppingCarts )
  {
    std::cout << "\n" << name << "'s shopping cart contains:\n";             // Print out the customer's name and the contents of their shopping cart
    todaysSales.merge( ringUpCustomer( shoppingCart, receipt ) );    // Ring up each customer accumulating the groceries purchased
  }
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  double amountDue = 0.0;    // 1 Initialize the amount due to zero

  for( const auto & [upc, temp_item] : shoppingCart )    // 2 For each grocery item in the customer's shopping cart
  {
    const GroceryItem * item = worldWideGroceryDatabase.find( upc );

    if( item == nullptr )
    {
      receipt << "\t\"" << upc << "\" (" << temp_item.productName() << ") not found, the item is free!\n";    // 2.1 If the item is not found in the world wide grocery item database, indicate on the receipt it's free of charge
    }
    else    // 2.2 Otherwise
    {
      receipt << "\t" << *item << "\n ";    // 2.2.1 Print the grocery item's full description on the receipt
      amountDue += item->price();               // 2.2.2 Add the grocery item's price to the amount due

      if( _inventoryDB.contains( upc ) )    // 2.2.3 If the grocery item is something the store sells (the item is in the store's inventory)
      {
        _inventoryDB[upc] -= 1;              // 2.2.3.1 Decrease the number of items on hand for the item sold
        purchasedGroceries.insert( upc );    // 2.2.3.2 Add the items's UPC to the list of groceries purchased
      }
    }
  }

  receipt << "\t-------------------------\n\tTotal  $" << std::fixed << std::setprecision( 2 ) << amountDue << "\n";    // 3 Print the total amount due on the receipt
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  for( const auto & upc : todaysSales )    // 1 For each grocery item sold today
  {
    if( !_inventoryDB.contains( upc ) || _inventoryDB[upc] < REORDER_THRESHOLD )    // 1.1 If the grocery item is not in the store's inventory or if the number of grocery items on hand has fallen below the re-order threshold (REORDER_THRESHOLD)
    {
      itemsOrdered++;
      const GroceryItem * item = worldWideGroceryDatabase.find( upc );

      if( item == nullptr ) reorderReport << '"' << upc << "\" not found in the world wide grocery item database\n";    // 1.1.1 If the grocery item is not in the world wide grocery item database, display just the UPC
      else
        reorderReport << "\n " << itemsOrdered << ":  {" << *item << "}\n";    // 1.1.2 Otherwise, display the grocery item's full description

      if( !_inventoryDB.contains( upc ) ) reorderReport
                                              << "        *** no longer sold in this store and will not be re-ordered\n";    // 1.1.3 If the grocery item is not in the store's inventory, display a notice indicating the grocery item is no longer sold in this store and will not be re-ordered
      else                                                                                                                   // 1.1.4 Otherwise,
      {
        reorderReport << "        only " << _inventoryDB[upc] << " remain in stock which is " << REORDER_THRESHOLD - _inventoryDB[upc] << " unit(s) below reorder threshold (" << REORDER_THRESHOLD << "), re-ordering " << LOT_COUNT << " more\n";    // 1.1.4 Display the current quantity on hand and the quantity re-ordered
        _inventoryDB[upc] += LOT_COUNT;                                                                                                                                                                                                                // 1.1.4 Increase the quantity on hand by the number of items ordered and received (LOT_COUNT)
      }
    }
  }

  todaysSales.clear();    // 2 Reset the list of grocery item sold today so the list can be reused again later
