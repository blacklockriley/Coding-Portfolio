...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  GroceryItem item;
  while( fin >> item ) _data.insert({ item.upcCode(), item }); // Read grocery items until end of file pushing each grocery item into the data store as they're read
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
GroceryItem * GroceryItemDatabase::find( const std::string & upc )    // Returns a pointer to the item in the database if found, nullptr otherwise
{
  auto it = _data.find( upc );                         // Delegate to the std::map's binary search function find()
  return it == _data.end() ? nullptr : &it->second;    // Return a pointer to the item in the database if found, nullptr otherwise
}

std::size_t GroceryItemDatabase::size() const    // Returns the number of items in the database
{
  return _data.size();    // Return the number of items in the database
}
