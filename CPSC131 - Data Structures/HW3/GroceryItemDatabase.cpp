...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  GroceryItem item;
  while( fin >> item ) _database.push_back( std::move(item) );
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
{
  return find( upc, _database.begin() );    // Call the recursive helper function
}

GroceryItem * GroceryItemDatabase::find( const std::string & upc, std::vector<GroceryItem>::iterator count )    // Recursive helper function for find()
{
  if( count == _database.end() ) return nullptr;    // Base Case: At the end and still no item, giving up
  if( count->upcCode() == upc ) return &*count;     // Base case: We found it! Return a pointer to it!
  return find( upc, count + 1 );                    // Recursive case:  Keep searching!
}

std::size_t GroceryItemDatabase::size() const
{
  return _database.size();    // Returns the number of items in the database
}
