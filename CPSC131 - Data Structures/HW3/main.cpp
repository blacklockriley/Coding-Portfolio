...
namespace
{
  ...
  *********************************************************************************************************************************/
  void carefully_move_grocery_items( std::size_t quantity, std::stack<GroceryItem> & broken_cart, std::stack<GroceryItem> & working_cart, std::stack<GroceryItem> & spare_cart )
  {
    ...
    std::ostream & str = std::clog;    // Use the standard error stream for tracing the moves.

    if( quantity == 1 )
    {
      working_cart.push( std::move( broken_cart.top() ) );    // Move top item from broken_cart to working_cart.
      broken_cart.pop();                                      // Remove the topmost grocery item from broken_cart.
      trace( broken_cart, working_cart, spare_cart, str );    // Trace the move.
    }
    else
    {
      carefully_move_grocery_items( quantity - 1, broken_cart, spare_cart, working_cart );    // Move top item from broken_cart to spare_cart.
      working_cart.push( std::move( broken_cart.top() ) );                                    // Move top item from broken_cart to working_cart.
      broken_cart.pop();                                                                      // Remove the topmost grocery item from broken_cart.
      trace( broken_cart, working_cart, spare_cart, str );                                    // Trace the move.
      carefully_move_grocery_items( quantity - 1, spare_cart, working_cart, broken_cart );    // Move top item from spare_cart to working_cart.
    }
  ...
  void carefully_move_grocery_items( std::stack<GroceryItem> & from, std::stack<GroceryItem> & to )
  {
    ...
    if( !from.empty() )    // Check if the "from" cart contains grocery items
    {
      std::stack<GroceryItem> spare_cart;                                   // Create a spare cart.
      trace( from, to, spare_cart );                                        // Call the trace function to get a starting point reference in the movement report.
      carefully_move_grocery_items( from.size(), from, to, spare_cart );    // Call the above carefully_move_grocery_items function to start moving grocery items recursively.
    }
...
int main( int argc, char * argv[] )
{
  try
  {
    ...
    std::stack<GroceryItem> myCart;    // Found an empty cart!
    ...
    myCart.push( GroceryItem( "milk", "any", "00075457129000" ) );                   // Put the milk on the bottom.
    myCart.push( GroceryItem( "rice krispies", "Kellogg's", "00038000291210" ) );    // Put the rice krispies on top of the milk.
    myCart.push( GroceryItem( "hotdogs", "Applegate Farms", "00025317533003" ) );    // Put the hotdogs on top of the rice krispies.
    myCart.push( GroceryItem( "apple pie", "any", "09073649000493" ) );              // Put the apple pie on top of the hotdogs.
    myCart.push( GroceryItem( "bread", "any", "00835841005255" ) );                  // Put the bread on top of the apple pie.
    myCart.push( GroceryItem( "eggs", "any", "00688267039317" ) );                   // Put the eggs on top of the bread.
    ...
    std::stack<GroceryItem> workingCart;                    // Found a working cart!
    carefully_move_grocery_items( myCart, workingCart );    // Carefully move the grocery items in your now broken cart to this working cart.
    ...
    std::queue<GroceryItem> checkoutCounter;    // Found a checkout lane with no one in line!
    while( !workingCart.empty() )               // Repeat until your cart is empty, you have to pay for everything in your cart.
    {
      checkoutCounter.push( std::move( workingCart.top() ) );    // Place the grocery items on the conveyor belt, starting from the top of the cart.
      workingCart.pop();                                         // Remove the topmost grocery item from your cart.
    }
    ...
    while( !checkoutCounter.empty() )    // Repeat until the checkout counter is empty, no item left behind!
    {
      GroceryItem * item = worldWideDatabase.find( checkoutCounter.front().upcCode() );
      if( item != nullptr )
      {
        amountDue += item->price();    // Accumulate the amount due.
        std::cout << *item << '\n';    // Print the grocery item's full description and price on the receipt.
      }
      else    // If the item is not found in the database
      {
        std::cout << "\"" + checkoutCounter.front().upcCode() + "\" (" << checkoutCounter.front().productName() << ") not found, so today is your lucky day - You get it free! Hooray!\n";    // Print a message on the receipt that a description and price for the grocery item wasn't found and there will be no charge.
      }
      checkoutCounter.pop();    // Remove the grocery item from the checkout counter.
    }
