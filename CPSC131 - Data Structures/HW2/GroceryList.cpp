...
std::size_t GroceryList::find( const GroceryItem & groceryItem ) const
{
  ...
  return std::find(_gList_vector.begin(), _gList_vector.end(), groceryItem) -
         _gList_vector.begin();
  /////////////////////// END-TO-DO (2) ////////////////////////////
}
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ...
    if (_gList_array_size >= _gList_array.size())
      throw CapacityExceeded_Ex(
          make_details("Insertion exceeds list capacity"));

    std::shift_right(_gList_array.begin() + offsetFromTop, _gList_array.begin() + _gList_array_size + 1, 1);
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ...
    std::shift_left(position, _gList_array.begin() + _gList_array_size, 1);
    --_gList_array_size;
  ...
  {/**********  Part 4 - Remove from singly linked list  **********/
    ///////////////////////// TO-DO (11) //////////////////////////////
      ...
      for( size_t i = 0; i < offsetFromTop; i++ ) std::advance(position, 1);
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    for(size_t i = 0; i < std::min(_gList_vector.size(), rhs._gList_vector.size()); i++) {
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
    ...
    for( auto p = _gList_vector.begin(), end = this->_gList_vector.end(), q = rhs._gList_vector.begin();  p != end;  ++p, ++q )
    {
      if( *p != *q ) return false;
    }
    return true;
